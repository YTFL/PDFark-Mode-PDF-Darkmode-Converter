...
interface PDFarkHeaderProps {
  /** When true, renders as a compact horizontal nav bar with a click-to-home action */
  compact?: boolean;
  onLogoClick?: () => void;
}
...
export const PDFarkHeader = ({ compact = false, onLogoClick }: PDFarkHeaderProps) => {
  if (compact) {
    return (
      ...
      >
        <div className="p-1 rounded-xl bg-white/5 overflow-hidden transition-colors group-hover:bg-white/10">
          <LogoSVG className="w-8 h-8" />
        </div>
        <span className="text-lg font-bold tracking-tight text-white transition-opacity group-hover:opacity-80">
          PDFark<span className="text-primary">Mode</span>
        </span>
      </button>
    );
  }
  ...
  return (
    <header className="flex flex-col items-center gap-2 mb-12">
      <div className="p-1 rounded-2xl bg-white/5 overflow-hidden">
      ...
      <p className="text-muted-foreground text-center max-w-md">
        Instant, client-side AMOLED dark mode conversion for your PDF documents.
      </p>
    </header>
  );
};
